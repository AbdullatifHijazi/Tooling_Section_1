...
void Reset_Handler(void){
 /* 1- copy Data Section from flash to SRAM      */
	uint32_t Dot_Data_Section_Size = (uint32_t)&_edata - (uint32_t)&_Sdata ; /* Get Size for .Data Section */
	
	uint8_t* SourcePtr = (uint8_t*)&_EText;                                  /* Start of .Data Section in Flash */
	uint8_t* DistinationPtr = (uint8_t*)&_Sdata;                             /* Start of .Data Section in SRAM */
	uint32_t Iterator ;
	for(Iterator = 0 ; Iterator < Dot_Data_Section_Size ; Iterator++)
	{
		*DistinationPtr = *SourcePtr ;
		DistinationPtr++;
		SourcePtr++;
	}
	
 /* 2- Initialise .bss Section in SRAM to Zero   */
    uint32_t Dot_Bss_Section_Size = (uint32_t)&_ebss - (uint32_t)&_sbss ; /* Get Size for .Bss Section */ 
    DistinationPtr = (uint8_t*)&_sbss;/* Start of .Bss Section in SRAM */
    Iterator ;
	for(Iterator = 0 ; Iterator < Dot_Data_Section_Size ; Iterator++)
	{
		*DistinationPtr = 0u;
